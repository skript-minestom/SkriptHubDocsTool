            ...
            if (values != null)
                label: for (Class<?> c : events) {
                    for (Object eventValue : values) {
                        Class<?> event = ReflectionUtils.getFieldOrElse(eventValue.getClass(), eventValue, "eventClass", "event");
                        if (event != null && (c.isAssignableFrom(event) || event.isAssignableFrom(c))) {
                            ...
                            Class<?> ret = ReflectionUtils.getFieldOrElse(eventValue.getClass(), eventValue, "valueClass", "c");
                            if (ret != null) {
                                clazz.add(ret);
                            }
                        }
